...
void SS::doFabricateSQL(int asz, char **theTokens, char **replacements, char *SQLt, char *cStringBuffer, int bufsize, bool debug) {
    ...
    inputTemplate.insert(resultingStringLength, 3, '\0'); //Pop in three characters of value 0 (not character zer0 \
    ...
    type c_string so C programs, which are oblivious to c++ type standard string, and lack a c-string-terminating '\0' character, \
    can use this data.
    kk = 0;
...
SS::SS(int n, char *templatePointer, int bufsz, char **tokens, char **replacements, bool debug) {
    i = j = kk = zero;
    caller_supplied_buffer_size = bufsz;  //This is the size of the output buffer where we will construct the resulting c-string with the template's tokens replaced with the values found in the replacements array.
    number_of_tokens_passed_by_calling_program = n; //There is a one-to-one relationship between the tokens in array tokens and in the replacements array.
    debugMode = debug;
    for (i = 0; i < n; i++) {
        ptrptrcopyOfReplacements[i] = &replacements[i];
        ptrcopyOfReplacements[i] = replacements[i];
        
        if (debug) cout << "A. sr_or_ss: " << sr_or_ss << "; " << ";\tptrcopyOfReplacements[" << i << "]: " << ptrcopyOfReplacements[i] << endl;
    ...
    if(templatePointer) { //Did caller supply a template pointer?
        ...
        memcpy(workArrayTemplate, templatePointer, lengthOfCallersTemplate);
/*        while ( templatePointer[kk] != '\0' || kk > caller_supplied_buffer_size ) { //keep on trucking until we reach the \
            c-string's zero-terminating character{
            workArrayTemplate[kk] = templatePointer[kk]; //Fill char array with caller-supplied SQL template. \
            workArrayTemplate is a c-string.
            kk++;
        }
 */
        inputTemplate.clear(); //Let's start with a clean input Template.
        inputTemplate.append( workArrayTemplate) ; //Input Template is of data type string. You just can't simply assign a c-string to a standard string string!!
        ...
        for (kk=zero; kk < templateSize; kk++) workArrayTemplate[kk] = inputTemplate[kk];
