...
private:
    ...
    (SELECT avg(id) FROM tbl_sun_loc_site WHERE lt::DATE='_DATE_' AND siteid=SITEID AND events IS NULL))   ) \
    ORDER BY id;";
    const int len_sunsetSQL_template = (int)default_template.length(); //Static is NOT needed because length of default_template is determined during \
    the execution phase.
...
public:
    ...
    char workArrayRepl[arrayBufferSize];
    char *ptr_workArrayRepl;
    char workArrayToken[arrayBufferSize];
    char *ptrOfCopyOfTokenArray[arrayOfCopy];
    char *ptrOfCopyOfReplacementArray[arrayOfCopy];
    ...
    string work;
    string work2;
    string stdStringArrayOfReplacementStrings[16];
    unsigned int lengthOfCallersTemplate;
    const char *sr_or_ss = "SRorSS";
    char cstring_workArray[16];
    char *ptr_workArray;
    char **ptrptrcopyOfReplacements[8];
    char **ptrptrcopyOfTokens[8];
    char *ptrcopyOfReplacements[8];
    char *ptrcopyOfTokens[8];
    ...
    int caller_supplied_buffer_size = 0;
    int cStringLength;
    int rc;
    char workArrayTemplate[templateBufferSize]; //Space reserved for holding, as a c-string, standard string found in  default_template, above.
